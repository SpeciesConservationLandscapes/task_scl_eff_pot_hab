...
class SCLPolygons(SCLTask):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # structural_habitat, _ = self.get_most_recent_image(
        ...
        self.set_aoi_from_ee("{}/{}/sumatra_poc_aoi".format(self.ee_rootdir, self.species))
    ...
    def calc(self):
        ...
        current_range = self.extirpated.updateMask(self.historic_range.eq(0)).eq(1).selfMask()
        connected_structural_habitat = str_hab_patch.updateMask(current_range)
        potential_habitat = connected_structural_habitat.updateMask(self.hii.lt(self.thresholds["hii"])).selfMask()
        excluded_habitat = connected_structural_habitat.updateMask(self.hii.gte(self.thresholds["hii"])).selfMask()
        ...
        minPatchSize = self.density.remap(self.remap_lists["density"], self.remap_lists["min_patch_size"]) \
            .updateMask(hii_ecoregion)
        minStpStnSize = self.density.remap(self.remap_lists["density"], self.remap_lists["min_stepping_stone_size"]) \
            .updateMask(hii_ecoregion)
        ...
        scl_eff_pot_hab = "{}/geographies/Sumatra/scl_poly/{}/scl_eff_pot_hab".format(self.species, self.taskdate)
